...
<head>
	...
	<script type="text/javascript">
        ...
        function setUrl(selStr,id) {
            var url = '${ctx}/staff/staffEval/list?staffId='+id+'&startData='+$("#beginDate").val()+'&endData='+$("#endDate").val();
...
<body>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><th>归属公司</th><th>归属部门</th><th>归属组</th><th class="sort-column name">姓名</th><th class="sort-column evalCount">评价次数</th><th class="sort-column taskCount">工作任务条数</th><th class="sort-column bugCount">生产缺陷数量</th><%--<th class="sort-column bugPoint">质量百分比</th>--%><th class="sort-column AvgEfficiency">任务延期次数</th><th class="sort-column goodCount">奖励总数</th><th class="sort-column badCount">惩罚总数</th><%--<th>角色</th> --%><shiro:hasPermission name="staff:archives:edit"><th>操作</th></shiro:hasPermission></tr></thead>
		<tbody>
		<c:forEach items="${page.list}" var="userEval" varStatus="vs">
			<tr>
				<td>${userEval.company.name}</td>
				<td>${userEval.item.name}</td>
				<td>${userEval.group.name}</td>
				<%--<td><a href="${ctx}/staff/staffEval/list?staffId=${user.id}">${user.loginName}</a></td>--%>
				<td>${userEval.name}</td>
				<td>${userEval.evalCountStr}</td>
				<td>${userEval.taskCountStr}</td>
				<td>${userEval.bugCountStr}</td>
				<%--<td>${empty userEval.bugPoint ? 0 : userEval.bugPointStr}</td>--%>
				<td>${userEval.avgEfficiencyStr}</td>
				<td>${userEval.goodCountStr}</td>
				<td>${userEval.badCountStr}</td><%--
				...
				<shiro:hasPermission name="staff:archives:view"><td>
					...
					<%--<a href="${ctx}/staff/staffEval/list?staffId=${userEval.id}">查看</a>--%>
					<a id="userA${vs.index}A" href="" onclick="setUrl('#userA${vs.index}A','${userEval.id}')">查看</a>
