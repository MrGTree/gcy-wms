...
<html>
<head>
	<title>离职员工基本信息管理</title>
	<meta name="decorator" content="default"/>
	<script type="text/javascript">
		$(document).ready(function() {
			$("#btnExport").click(function(){
				top.$.jBox.confirm("确认要导出离职员工信息表吗？数据量大请耐心等待","系统提示",function(v,h,f){
					if(v=="ok"){
		...
		function page(n,s){
			if(n) $("#pageNo").val(n);
			if(s) $("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/staff/staffArchives/leaveList");
			$("#searchForm").submit();
	    	return false;
	    }
	</script>
...
<body>
	...
	<ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="sys:user:viewLeave"><li class="active"><a href="${ctx}/staff/staffArchives/leaveList">离职员工基本信息列表</a></li></shiro:hasPermission>
	...
	<form:form id="searchForm" modelAttribute="user" action="${ctx}/staff/staffArchives/leaveList" method="post" class="breadcrumb form-search ">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<input id="leaveFlag" name="leaveFlag" type="hidden" value="1"/>
		...
		<ul class="ul-form">
			...
			<li class="btns"><input id="btnSubmit" class="btn btn-primary" type="submit" value="查询" onclick="return page();"/>
				<input id="btnExport" class="btn btn-primary" type="button" value="导出信息"/>
