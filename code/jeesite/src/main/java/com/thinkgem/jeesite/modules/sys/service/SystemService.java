...
public class SystemService extends BaseService implements InitializingBean {
	...
	public Page<User> findLeaveUser(Page<User> page, User user) {
		// 生成数据权限过滤条件（dsf为dataScopeFilter的简写，在xml中使用 ${sqlMap.dsf}调用权限SQL）
		user.getSqlMap().put("dsf", dataScopeFilter(user.getCurrentUser(), "o", "a"));
		// 设置分页参数
		user.setPage(page);
		// 执行分页查询
		page.setList(userDao.findLeaveList(user));
		return page;
	}
	 ...
	 * 分页查询用户数据但是不包含自己
	 */
	public Page<UserEval> findUserNotSelf(Page<UserEval> page, UserEval userEval) {
		// 生成数据权限过滤条件（dsf为dataScopeFilter的简写，在xml中使用 ${sqlMap.dsf}调用权限SQL）
		userEval.getSqlMap().put("dsf", dataScopeFilter(userEval.getCurrentUser(), "o", "a"));
		// 设置分页参数
		userEval.setPage(page);
		// 设置默认时间范围，默认当前月和上3个月
		if (userEval.getEndDate() == null){
			//DateUtils.setDays(DateUtils.parseDate(DateUtils.getDate()), 1)
			userEval.setEndDate(DateUtils.parseDate(DateUtils.getDate()));
		}
		if (userEval.getBeginDate() == null){
			userEval.setBeginDate(DateUtils.addMonths(userEval.getEndDate(), -3));
		}

		// 执行分页查询
		page.setList(userDao.findListNotSelf(userEval));
		return page;
	}

	public List<UserEval> findUserNotSelfNoPage(UserEval userEval) {
		// 生成数据权限过滤条件（dsf为dataScopeFilter的简写，在xml中使用 ${sqlMap.dsf}调用权限SQL）
		userEval.getSqlMap().put("dsf", dataScopeFilter(userEval.getCurrentUser(), "o", "a"));
		// 设置默认时间范围，默认当前月和上3个月
		if (userEval.getEndDate() == null){
		...
		List<UserEval> listNotSelf = userDao.findListNotSelf(userEval);
...
//		systemRealm.clearAllCachedAuthorizationInfo();
    ...
    public List<StaffPropTemplate> getTemplateListByTemplateIdAndRoleId(String templateId, List<String> roleIdList) {
		return menuDao.getTemplateListByTemplateIdAndRoleId(templateId,roleIdList);
    }

    public List<StaffPropTemplate> getTemplateListAndAnswerByTemplateIdAndRoleId(String templateId,String staffId,String UUID, List<String> roleIdList) {
		return menuDao.getTemplateListAndAnswerByTemplateIdAndRoleId(templateId,staffId,UUID,roleIdList);
    }

    public List<StaffPropTemplate> getAllTemplateListAndAnswerByTemplateId(String templateId,String staffId,String UUID) {
		return menuDao.getAllTemplateListAndAnswerByTemplateId(templateId,staffId,UUID);
    }



	public List<StaffPropTemplate> getTemplateListByTemplateId(String templateId) {
		return menuDao.getTemplateListByTemplateId(templateId);
	}

    public List<StaffPropTemplate> getAllTemplateList() {
		return menuDao.getAllTemplateList();
    }

	@Transactional(readOnly = false)
	public void leaveUser(User user) {
		userDao.updateLeaveFlag(user);
		// 同步到Activiti
		deleteActivitiUser(user);
		// 清除用户缓存
		UserUtils.clearCache(user);
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	@Transactional(readOnly = false)
	public void comeUser(User user) {
		// 清除原用户机构用户缓存
		User oldUser = userDao.get(user.getId());
		if (oldUser.getOffice() != null && oldUser.getOffice().getId() != null) {
			CacheUtils.remove(UserUtils.USER_CACHE, UserUtils.USER_CACHE_LIST_BY_OFFICE_ID_ + oldUser.getOffice().getId());
