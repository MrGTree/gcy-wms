...
public class DeskCreamerUtil {
    ...
    private static Map<String,List<DeskCreamer>> map = SpringContextHolder.getBean("deskCreamerMaps");

    public static DeskCreamer calcAndGetDeskCreamer(String creamerName, int x , int y){
        List<DeskCreamer> deskCreamers = map.get(creamerName);
        DeskCreamer minDistDC = getMinDistDC(deskCreamers, x, y);
        return minDistDC;
    }

    private static DeskCreamer getMinDistDC(List<DeskCreamer> deskCreamers, int x , int y){
